...
{
   // Check for dominance
   //cout << "plan->ordering: " << plan->ordering << endl;
   if (0) {
   if (~plan->ordering) {
      Plan* last=0;
   ...
   // Apply filters to index scans
{
   ...
   cout << "filter size: " << query.filters.size() << endl;
   ...
   // Build an index scan
{
   ...
   if (!~value1) {
     ...
     //cout << "here order:" << order << endl;
      ...
      //cout << "plan ordering==value1: " << value1 << endl;
   }
   ...
   // Generate base table accesses
{
  //cout << "build scan!\n";
   ...
   cout << "unusedSubject: " << unusedSubject << " unusedPredicate: " << unusedPredicate << " unusedObject: " << unusedObject << endl;
   ...
   for (Plan* iter=result->plans;iter;iter=iter->next) {
      Plan* iter2=iter;
      iter->sum_cardinality = iter->cardinality;
   ...
   // Translate a query into an operator tree
{
   ...
   for (vector<QueryGraph::Node>::const_iterator iter=query.nodes.begin(),limit=query.nodes.end();iter!=limit;++iter,++id) {
      cout << "s: " << iter->subject << " p: " << iter->predicate << " o: " << iter->object << endl;
   ...
   cout << "dpTable size: " << dpTable.size() << endl;
   cout << "table function size: " << query.tableFunctions.size() << endl;
   cout << "query.edges size: " << query.edges.size() << endl;
		 ...
		 bool test;
               ...
               for (Plan* leftPlan=iter->plans;leftPlan;leftPlan=leftPlan->next) {
                  for (Plan* rightPlan=iter2->plans;rightPlan;rightPlan=rightPlan->next) {
                    //cout << "leftPlan print:\n" << endl;
                    //leftPlan->print(0);
                    //cout << "leftPlan print finish:\n" << endl;
                    //cout << "rightPlan print:\n" << endl;
                    //rightPlan->print(0);
                    //cout << "rightPlan print finish:\n" << endl;
...
*/
 			       ...
 			       p->costs=(leftPlan->costs+rightPlan->costs)+Costs::mergeJoin(leftPlan->cardinality,rightPlan->cardinality);
                               ...
                               p->sum_cardinality = leftPlan->cardinality + rightPlan->cardinality + p->cardinality;
                               addPlan(problem,p);
                               break;
                            }
                         }
...
//    plancount ++;
//  }
   cout << "plan process finish!\n" << endl;
   //cout << "plan size count:" << plancount << endl;
