...
bool TriADProcessor::executePerNodeQuery(Channel ch, vector<Channel> & channels, vector<unsigned> & results, unsigned& numVars) {
	...
	if(plan){
		...
		if(output){
				...
				LOG4CXX_INFO(logger, "Node-" << rank << ": sum Result size: " << output->sumTuples);
		}
		else{
...
void TriADProcessor::startQueryTask(vector<Channel> channels) {
	...
	//rg::Timer query_time;
  uint64_t count = 0;
    vector<VECTOR_INT_32> all_plans;

    uint64_t beginp = 0;
    ifstream ifs("countfile");
    ifs >> beginp;
    ifs.close();
	...
	while (true) {
		//string query;
    if (count !=0 && count == all_plans.size()) {
      cout << "finish all plans\n";
      break;
    }
    if (count == 0) {
...
//		t1.stop();
		...
		tmp.start();
		if(count == 0) ret = qEngine->planGenerator(query, all_plans, plan_plist,summary);
		tmp.stop();
		LOG4CXX_INFO(logger, "Plan generation time " << tmp);
    if(count == 0) cout << "all plans size: " << all_plans.size() << endl;
    plan_serialzied = all_plans[count];
    cout << "plan " << count << ":" << endl;
		if (ret) {
			tmp.start();
