...
Relation* QueryEngine::executeQuery(Database *db, Plan* plan, vector<Channel> &channels, unsigned rank, bool full){
    ...
    scan->sumTuples = scan->getSize();
    //cout << "sumTuples: " << scan->sumTuples << " num" << scan->numTuples << endl;
		return scan;
	}else{
		...
		if(plan->op == Plan::MergeJoin){
			...
			if(left && right){
				...
				timer.resume();
        //cout << "left->sumTuples " << left->sumTuples << " right: " << right->sumTuples << endl;
        mjoin->sumTuples = mjoin->getSize() + left->sumTuples + right->sumTuples;
				return mjoin;
			}
      ...
      //cout << "prepare for hash join\n";
        ...
        //cout << "finish hj\n";
				...
				timer.resume();
        hjoin->sumTuples = hjoin->getSize() + left->sumTuples + right->sumTuples;
        //cout << "hash join finish\n";
				return hjoin;
			}
		}
	}
...
bool QueryEngine::planGenerator(string &query,vector<VECTOR_INT_32>& plan_serialize, VECTOR_INT_32 &plist, bool mode)
   // Evaluate a query
{
	...
	QueryGraph queryGraph;
  {
     ...
     if (queryGraph.knownEmpty()) {
      ...
      // sort the plan by the cost
      vector<Plan*> order_plans;
      vector<Plan*> dup_plans;
      while (plan) {
      Plan* best=0;
      Plan* last = 0;
      Plan* record = 0;
      ...
      dup_plans.push_back(best);
      if (record)
        record->next=best->next;
      else plan = best->next;
      }
      cout << "dup_plans size: " << dup_plans.size() << endl;

      // remove dup
      Plan* pre = 0;
      for (int yy = 0; yy < dup_plans.size(); yy++) {
        Plan* tmpp = dup_plans[yy];
        if(pre && (tmpp->costs == pre->costs) && (tmpp->sum_cardinality == pre->sum_cardinality)) 
          continue;
        order_plans.push_back(tmpp);
        pre = tmpp;
      }

      cout << "after dedup order_plans size: " << order_plans.size() << endl;

//for (Plan* tmpp=plan;tmpp;tmpp=tmpp->next) {
for (int pp = 0; pp < order_plans.size(); pp++) {
      Plan* tmpp = order_plans[pp];
      cout << "-----------------------------------\n";
      cout << "plan " << pp << ":" << endl;
      tmpp->print(0);
      cout << "estcosts: " << tmpp->costs << endl;
      cout << "total card: " << tmpp->sum_cardinality << endl;
      VECTOR_INT_32 tmp_plan_ser;
      unsigned id = 1;
      serializePlan(tmpp, tmp_plan_ser,id);
      plan_serialize.push_back(tmp_plan_ser);
}
      //if (!plan) {
      //   cerr << "internal error plan generation failed" << endl;
      //   return 0;
